...
from database_01 import public_price_table, DB_CONNECTION_STRING
class ServiceProvider(Agent):
    def __init__(self, unique_id, model, db_connection_string):
    ...
    def check_shared_availability(self, company_name, start_time):
        ...
        :param company_name: Name of the company providing the service.
        ...
        if start_time < current_step:
            print(f"The current step is {current_step} and the start time is {start_time}")
            raise ValueError("start_time must be more than the current step")
        ...
        with self.Session() as session:
            try:
                ...
                if service and getattr(service, availability_column) > 0:
                    return True
                else:
                    return False
            except SQLAlchemyError as e:
    ...
    def get_public_service_price(self, mode, time, public_price_table = public_price_table):
        try:
            if mode in public_price_table:
                if self.check_is_peak(time):
                    return public_price_table[mode]['on_peak']
                else:
                    return public_price_table[mode]['off_peak']
            else:
    ...
    def get_shared_service_price(self, mode, start_time):
        ...
        step_count_to_query = current_step - 1
    ...
    def record_booking_log(self, commuter_id, request_id, mode, start_time, duration, affected_steps, route_details):
        service_name = mode.split('_')[0]
        company_name = mode.split('_')[1]
        
        provider_table = self.get_service_table(company_name)
        if provider_table is None:
            print(f"No provider table found for service name: {service_name}")
            return
    ...
    def dynamic_pricing_share(self):
        try:
            with self.Session() as session:
                service_tables = ['UberLike1', 'UberLike2', 'BikeShare1', 'BikeShare2']
                alpha = 0.5  # Surge multiplier coefficient
                ...
                for service_table_name in service_tables:
                    ...
                    for service in session.query(service_table).all():
                        for i in range(6):
                            ...
                            surge_multiplier = alpha * ((SDR - 1) / SDR_threshold)
                            ...
                            gamma = 1.0 # Don't include this for now
                            # location = self.get_location(service)
                            # if self.is_high_demand_area(location):
                            #     gamma = 1.2
                            # else:
                            #     gamma = 1.0

                            # Calculate Dynamic Price
                            dynamic_price = service.base_price * (1 + surge_multiplier) * beta * gamma
    ...
    def fetch_availability_from_db(self, current_step):
        old_engine = create_engine(DB_CONNECTION_STRING)
