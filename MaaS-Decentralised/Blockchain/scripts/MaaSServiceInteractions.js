...
const maasCoreContractAddress = '0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9';
...
// Contract ABI
const ABI = [
    "function addCommuter((uint32 commuter_id, uint32 x_coordinate, uint32 y_coordinate, uint8 c_income, uint8 preferred_mode_id, uint32 age, bool has_disablility, bool tech_access, uint8 c_health))",
    "function addProvider((uint32 provider_id, address p_address, string mode_type))",
    "function addRequest((uint32 request_id, uint32 commuter_id, uint32 origin_x_coordinate, uint32 origin_y_coordinate, uint32 destination_x_coordinate, uint32 destination_y_coordinate, uint32 start_time, uint8 travelPurpose, uint8 status, uint32 winningOfferId))",
    "function getNumberCommuters() view returns (uint256)",
    "function getNumberProviders() view returns (uint256)",
    "function submitOffer((uint32 id, uint32 request_id, uint32 provider_id, uint256 price, uint8 mode, uint256 startTime, uint256 totalTime, uint256 totalPrice, uint256 totalLength))",
    "function getNumberRequests() view returns (uint256)",
    "function createAuction((uint256 auction_id, uint32 modeIndicator, string path, bool status, uint32 request_id, uint32 commuter_id, uint32 winner))"
];

// Initialize wallet and contract
const wallet = new ethers.Wallet(userPrivateKey, ethereumProvider);
const contract = new ethers.Contract(maasCoreContractAddress, ABI, wallet);
...
// Define the new offer object
const newOffer = {
    id: 1,
    request_id: 1,
    provider_id: 1,
    price: ethers.parseEther("0.1"),
    mode: 0, // Assuming 0 represents 'PublicTransport'
    startTime: 1234567890,
    totalTime: 120,
    totalPrice: ethers.parseEther("0.1"),
    totalLength: 10
};

// Function to add a commuter
async function addCommuter() {
    const tx = await contract.addCommuter(newCommuter);
    await tx.wait();
...
async function addProvider() {
    const tx = await contract.addProvider(newProvider);
    await tx.wait();
...
async function addRequest() {
    const tx = await contract.addRequest(newRequest);
    await tx.wait();
    ...
    const atx = await contract.createAuction(singlenewAuction);
    await atx.wait();
    console.log('atx :', atx);
}
...
async function submitOffer() {
    const tx = await contract.submitOffer(newOffer);
    await tx.wait();
    console.log('Offer submitted:', newOffer);
}

async function getNumberCommuters() {
    const tx = await contract.getNumberCommuters();
   // await tx.wait();
    console.log('number of commuters:', tx);
}
async function getNumberProviders() {
    const tx = await contract.getNumberProviders();
 //   await tx.wait();
    console.log('number of providers:', tx);
}
async function getNumberRequests() {
    const tx = await contract.getNumberRequests();
  //  await tx.wait();
    console.log('number of requests:', tx);
}
