...
def optimize_allocation_parallel(fps_value, base_parameters, num_cpus=NUM_CPUS):
    ...
    target_equity_threshold = 0.05  # kept for parity; you can disable/tune later
    ...
    for iteration in range(max_iterations):
        ...
        if no_improvement_iterations >= patience:
            return {
                ...
                "equity_scores": {"low": np.nan, "middle": np.nan, "high": np.nan},  # keep aggregator-safe
                "avg_equity": float(best_obj),  # aggregator expects avg_equity
                "total_system_travel_time": float(best_obj),
                "avg_trip_time": float(best_results.get("avg_trip_time", np.nan)) if best_results else np.nan,
                "total_trips": int(best_results.get("total_trips", 0)) if best_results else 0,
                "income_breakdown": {lvl: best_results.get(lvl, {}) for lvl in ["low", "middle", "high"]} if best_results else {},
                "mode_breakdown": best_results.get("mode_breakdown", {}) if best_results else {},
    ...
    return {
        "fps_value": float(fps_value),
        "optimal_allocations": best_allocations,
        "equity_scores": {"low": np.nan, "middle": np.nan, "high": np.nan},
