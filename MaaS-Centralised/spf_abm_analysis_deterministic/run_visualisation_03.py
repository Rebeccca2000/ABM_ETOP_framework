...
class MobilityModel(Model):
    def __init__(self, db_connection_string, num_commuters, grid_width, grid_height, data_income_weights , \
        ...
        self.deterministic_scenario_path = deterministic_scenario_path
        self.deterministic_scenario = deterministic_scenario
        self.deterministic_trip_plan_by_commuter = {}
        self.deterministic_background_traffic_by_step = {}
        scenario_commuters = None
        if self.deterministic_mode:
            scenario_data = self._load_deterministic_scenario()
            scenario_commuters = scenario_data.get("commuters", [])
            self.deterministic_trip_plan_by_commuter = {
                int(commuter_id): deque(trips)
                for commuter_id, trips in scenario_data.get("trip_plan", {}).items()
            }
            self.deterministic_background_traffic_by_step = {
                int(step): events
                for step, events in scenario_data.get("background_traffic", {}).items()
            }
            if scenario_commuters:
                self.num_commuters = len(scenario_commuters)
        ##################################################################################
        ...
        if self.deterministic_mode and scenario_commuters:
            for profile in scenario_commuters:
                self._add_commuter_agent(
                    unique_id=int(profile["unique_id"]),
                    commuter_location=(int(profile["location"][0]), int(profile["location"][1])),
                    age=int(profile["age"]),
                    income_level=str(profile["income_level"]),
                    has_disability=bool(profile["has_disability"]),
                    tech_access=bool(profile["tech_access"]),
                    health_status=str(profile["health_status"]),
                    payment_scheme=str(profile["payment_scheme"]),
    ...
    def _load_deterministic_scenario(self):
        if isinstance(self.deterministic_scenario, dict):
            return self.deterministic_scenario
        if self.deterministic_scenario_path:
            with open(self.deterministic_scenario_path, "r", encoding="utf-8") as f:
                return json.load(f)
        raise ValueError(
            "deterministic_mode=True requires deterministic_scenario_path or deterministic_scenario"
        )

    def _add_commuter_agent(
        self,
        unique_id,
        commuter_location,
        age,
    ...
    def create_preplanned_trip(self, current_step, commuter):
        trip_queue = self.deterministic_trip_plan_by_commuter.get(commuter.unique_id)
        if not trip_queue:
            return False

        if commuter.requests and not self.all_requests_finished(commuter):
            return False

        while trip_queue and int(trip_queue[0].get("release_step", 0)) < current_step:
            trip_queue.popleft()

        if not trip_queue:
            return False

        next_trip = trip_queue[0]
        release_step = int(next_trip.get("release_step", 0))
        if release_step > current_step:
            return False

        trip_queue.popleft()
        origin = commuter.location
        destination_raw = next_trip.get("destination", list(origin))
        destination = (int(destination_raw[0]), int(destination_raw[1]))
        start_delay = max(1, int(next_trip.get("start_delay", 1)))
        start_time = min(current_step + start_delay, current_step + 5)
        request_id = uuid.uuid4()
        travel_purpose = str(next_trip.get("purpose", "trip"))
    ...
    def create_time_based_trip(self, current_step, commuter):
        ...
        if self.deterministic_mode:
            return self.create_preplanned_trip(current_step, commuter)
