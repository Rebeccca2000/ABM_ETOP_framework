...
def calculate_equity_indicator(session, schema=None):
    ...
    try:
        ...
        for income_level in income_levels:
            ...
            total_trips_by_income[income_level] = sum(choice[1] for choice in mode_choices)
            ...
            for mode, count in mode_choices:
                ...
                # Store mode share
                denominator = total_trips_by_income[income_level]
                if denominator <= 0:
                    continue
                if category in mode_shares[income_level]:
                    mode_shares[income_level][category] += count / denominator
                else:
                    mode_shares[income_level][category] = count / denominator
        ...
        if all_trips > 0:
            income_weights = {income: total_trips_by_income[income] / all_trips for income in income_levels}
        else:
            income_weights = {income: 1.0 / len(income_levels) for income in income_levels}
...
def calculate_trip_diagnostics(session, schema=None):
    """Compute trip lifecycle diagnostics for cross-FPS comparability checks."""
    try:
        if schema:
            session.execute(text(f"SET search_path TO {schema}"))
            session.commit()

        total_records = int(session.execute(text("SELECT COUNT(*) FROM service_booking_log")).scalar() or 0)
        finished_records = int(
            session.execute(text("SELECT COUNT(*) FROM service_booking_log WHERE status = 'finished'")).scalar() or 0
        )
        expired_records = int(
            session.execute(text("SELECT COUNT(*) FROM service_booking_log WHERE status = 'expired'")).scalar() or 0
        )
        selected_records = int(
            session.execute(
                text("SELECT COUNT(*) FROM service_booking_log WHERE status = 'Service Selected'")
            ).scalar()
            or 0
        )

        by_income = {'low': 0, 'middle': 0, 'high': 0}
        income_rows = session.execute(
            text(
                """
                SELECT cil.income_level, COUNT(sbl.request_id) AS cnt
                FROM service_booking_log sbl
                JOIN commuter_info_log cil ON sbl.commuter_id = cil.commuter_id
                GROUP BY cil.income_level
                """
            )
        ).fetchall()
        for income_level, count in income_rows:
            by_income[str(income_level)] = int(count)

        return {
            'total_records': total_records,
            'finished_records': finished_records,
            'expired_records': expired_records,
            'service_selected_records': selected_records,
            'by_income': by_income,
        }
    except Exception as e:
        print(f"Error calculating trip diagnostics: {e}")
        return {
            'total_records': 0,
            'finished_records': 0,
            'expired_records': 0,
            'service_selected_records': 0,
            'by_income': {'low': 0, 'middle': 0, 'high': 0},
        }
...
def run_single_simulation(params):
    ...
    try:
        ...
        if pilot_steps is not None and best_equity is not None and np.isfinite(best_equity):
            ...
            pilot_trip_diag = calculate_trip_diagnostics(session, schema=schema_name)
            ...
            if exhausted_early and (pilot_equity > best_equity * (1.0 + margin)):
                return {
                    ...
                    'trip_diagnostics': pilot_trip_diag,
        ...
        trip_diagnostics = calculate_trip_diagnostics(session, schema=schema_name)
        results['subsidy_usage'] = subsidy_stats
        results['trip_diagnostics'] = trip_diagnostics
        ...
        if results:
            ...
            results['deterministic_mode'] = deterministic_mode
            results['deterministic_scenario_path'] = deterministic_scenario_path or ""
        ...
        return {
            ...
            'deterministic_mode': deterministic_mode if 'deterministic_mode' in locals() else False,
            'deterministic_scenario_path': deterministic_scenario_path if 'deterministic_scenario_path' in locals() else "",
            'total_equity_indicator': 0,
            'low': {'equity_indicator': 0, 'mode_shares': {}},
            'middle': {'equity_indicator': 0, 'mode_shares': {}},
            'high': {'equity_indicator': 0, 'mode_shares': {}},
            ...
            if equity_indicator < best_equity:
                ...
                if best_equity < target_equity_threshold:
                    ...
                    return {
                        ...
                        'trip_diagnostics': best_results.get('trip_diagnostics', {}),
                        'terminated_early': True,
