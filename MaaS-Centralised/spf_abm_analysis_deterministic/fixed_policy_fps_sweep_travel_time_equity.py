...
try:
    from spf_abm_analysis_deterministic.travel_time_equity_optimization import run_single_simulation
except ModuleNotFoundError:
    from travel_time_equity_optimization import run_single_simulation
...
def resolve_policy_from_opt_dir(opt_dir: str, policy_source_fps: float | None):
    ...
    if policy_source_fps is not None:
        ...
        ref_fps = min(
            normalized.keys(),
            key=lambda f: float(normalized[f].get("travel_time_equity_index", float("inf"))),
        )
...
def build_row(fps: float, result: dict):
    ...
    return {
        "fps_value": float(fps),
        "travel_time_equity_index": float(result.get("travel_time_equity_index", np.nan)),
        "avg_equity": float(result.get("avg_equity", np.nan)),
        "overall_avg_travel_time": float(result.get("overall_avg_travel_time", np.nan)),
        "total_travel_time": float(result.get("total_travel_time", np.nan)),
        "total_trips": float(result.get("total_trips", np.nan)),
        "total_deviation": float(result.get("total_deviation", np.nan)),
        "avg_deviation": float(result.get("avg_deviation", np.nan)),
        "low_avg_travel_time": float(result.get("low", {}).get("avg_travel_time", np.nan)),
        "low_deviation": float(result.get("low", {}).get("deviation", np.nan)),
        "low_trip_count": float(result.get("low", {}).get("trip_count", np.nan)),
        "middle_avg_travel_time": float(result.get("middle", {}).get("avg_travel_time", np.nan)),
        "middle_deviation": float(result.get("middle", {}).get("deviation", np.nan)),
        "middle_trip_count": float(result.get("middle", {}).get("trip_count", np.nan)),
        "high_avg_travel_time": float(result.get("high", {}).get("avg_travel_time", np.nan)),
        "high_deviation": float(result.get("high", {}).get("deviation", np.nan)),
...
def save_visualizations(df: pd.DataFrame, out_dir: str):
    ...
    plt.plot(
        df["fps_value"],
        df["travel_time_equity_index"],
        "o-",
        linewidth=2,
        label="Travel Time Equity Index",
    )
    plt.plot(df["fps_value"], df["low_deviation"], "o-", linewidth=1.5, label="Low")
    plt.plot(df["fps_value"], df["middle_deviation"], "o-", linewidth=1.5, label="Middle")
    plt.plot(df["fps_value"], df["high_deviation"], "o-", linewidth=1.5, label="High")
    plt.xscale("log")
    plt.grid(True, alpha=0.3)
    plt.xlabel("FPS")
    plt.ylabel("Deviation Index (Lower is Better)")
    plt.title("Fixed Policy Deterministic Sweep: Travel Time Equity vs FPS")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fixed_policy_travel_time_equity_vs_fps.png"), dpi=300)
    plt.close()
    ...
    ax2.plot(
        df["fps_value"],
        df["overall_avg_travel_time"],
        "s--",
        color="#d62728",
        label="Overall Avg Travel Time",
    )
    ax2.set_ylabel("Overall Avg Travel Time", color="#d62728")
    ax2.tick_params(axis="y", labelcolor="#d62728")

    ax1.set_title("Fixed Policy Deterministic Sweep: Trips and Travel Time vs FPS")
    fig.tight_layout()
...
def write_summary(
    ...
    with open(os.path.join(out_dir, "run_summary.txt"), "w", encoding="utf-8") as f:
        ...
        f.write("\nKey observations\n")
        f.write(
            f"- equity_min={rows_df['travel_time_equity_index'].min():.6f}, "
            f"equity_max={rows_df['travel_time_equity_index'].max():.6f}\n"
        )
        f.write(
        ...
        f.write(
            f"- overall_avg_travel_time_min={rows_df['overall_avg_travel_time'].min():.6f}, "
            f"overall_avg_travel_time_max={rows_df['overall_avg_travel_time'].max():.6f}\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run deterministic fixed-policy FPS sweep for travel time equity. "
